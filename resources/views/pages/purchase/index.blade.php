            ...
            <div class="col">
                ...
                <div class="page-title-container">
                    <div class="row">
                        ...
                        <div class="col-12 col-md-5 d-flex align-items-start justify-content-end">
                            ...
                            <a href="{{ route('purchase.create') }}" class="btn btn-outline-primary btn-icon btn-icon-start w-100 w-md-auto add-datatable">
                ...
                <div class="data-table-rows slim">
                    ...
                    <div class="data-table-responsive-wrapper">
                        <table id="DataTables" class="data-table nowrap w-100">
                            <thead>
                            <tr>
                                <th class="text-small text-uppercase">No</th>
                                <th class="text-small text-uppercase">Name</th>
                                <th class="text-small text-uppercase">Total</th>
                            ...
                            <tbody>
                                @foreach ($data as $item)
                                    <tr data-id="{{$item->id}}">
                                        <td>{{ $item->transno }}</td>
                                        <td>{{ $item->name == '' ? 'Tanpa Nama' :  $item->name }}</td>
                                        <td>{{ $item->total }}</td>
                                        <td>
                                            <a href="{{ route('purchase.invoice', ['id' => $item->id]) }}" target="blank" class="btn btn-sm btn-icon btn-icon-only btn-info mb-1" data-bs-delay="0" data-bs-toggle="tooltip" data-bs-placement="top" title="Show">
                                                <i data-acorn-icon="print"></i>
                                            </a>
                                            <a href="{{ route('purchase.edit', ['id' => $item->id]) }}" class="btn btn-sm btn-icon btn-icon-only btn-primary mb-1" data-bs-delay="0" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                <i data-acorn-icon="edit"></i>
                                            </a>
                                            {{-- Delete --}}
                                            <form action="{{ route('purchase.destroy', ['id' => $item->id]) }}" method="POST" class="d-inline-block">
                                                @csrf
                                                @method('DELETE')
