        ...
        <div class="row">
            <div class="col">
                <div class="card data-table-rows slim p-3">
                    <p id="title-head" class="mb-5 h3 text-center">Data Laporan Penjualan @if (session('start')){{ session('start') }} - @endif @if (session('end')){{ session('end') }}@endif</p>
                    <form id="filterReport" method="POST" action="{{ route('report.sales.show') }}">
                        @csrf
                        <div class="row mb-2">
